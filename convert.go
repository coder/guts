...
//
// Multiple calls for the same module specifier are merged into one import.
// Named specifiers are unioned by (Name, Alias, IsTypeOnly); duplicates are
// dropped while preserving the order of first occurrence. The merged import
// is marked IsTypeOnly only when every contributing import for that module
// is type-only.
