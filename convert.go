...
type Typescript struct {
	...
	preserveComments bool
	// imports holds top-level `import { ... } from "..."` statements that
	// are emitted before the named declarations during Serialize. Order is
	// the order of first AppendImport for each module; specifiers within a
	// module also keep their first-seen order.
	imports []*bindings.ImportDeclaration
	// Do not allow calling serialize more than once.
	// The call affects the state.
	serialized bool
}

// AppendImport adds (or merges into) a top-level import statement.
//
// Imports are keyed by (Module, IsTypeOnly, SideEffect): repeat calls with
// the same key are merged into one statement, mixed type-only and value
// imports for the same module emit as separate statements, and a
// side-effect import (SideEffect=true) deduplicates per module.
//
// Named specifiers are unioned by (Name, Alias, IsTypeOnly); duplicates are
// dropped while preserving the order of first occurrence. The final emitted
// order is sorted alphabetically by module (then by SideEffect, then by
// IsTypeOnly), and within each statement the specifiers are sorted by name.
func (ts *Typescript) AppendImport(decl *bindings.ImportDeclaration) {
	if decl == nil {
		return
	}
	for _, existing := range ts.imports {
		if existing.Module != decl.Module {
			continue
		}
		if existing.SideEffect != decl.SideEffect {
			continue
		}
		if existing.SideEffect {
			// Side-effect imports have no specifiers and no type-only mode,
			// so any same-module match is a complete duplicate.
			return
		}
		if existing.IsTypeOnly != decl.IsTypeOnly {
			continue
		}
		for _, n := range decl.Named {
			if !containsImportSpecifier(existing.Named, n) {
				existing.Named = append(existing.Named, n)
			}
		}
		return
	}
	ts.imports = append(ts.imports, decl)
}

func containsImportSpecifier(list []*bindings.ImportSpecifier, target *bindings.ImportSpecifier) bool {
	for _, s := range list {
		if s.Name == target.Name && s.Alias == target.Alias && s.IsTypeOnly == target.IsTypeOnly {
			return true
		}
	}
	return false
...
// sortImports stabilizes the emission order for the imports list and the
// specifiers within each entry. Without this the output would depend on the
// order ApplyMutations was called, which is brittle for diff review and
// makes the generated file flaky to regenerate.
//
// Imports are sorted by (Module, !SideEffect, IsTypeOnly): same module
// groups together, side-effect import first inside the group, then value
// import, then type-only import. Specifiers within an import are sorted by
// (Name, Alias, IsTypeOnly).
func (ts *Typescript) sortImports() {
	sort.SliceStable(ts.imports, func(i, j int) bool {
		a, b := ts.imports[i], ts.imports[j]
		if a.Module != b.Module {
			return a.Module < b.Module
		}
		if a.SideEffect != b.SideEffect {
			return a.SideEffect
		}
		return !a.IsTypeOnly && b.IsTypeOnly
	})
	for _, imp := range ts.imports {
		sort.SliceStable(imp.Named, func(i, j int) bool {
			a, b := imp.Named[i], imp.Named[j]
			if a.Name != b.Name {
...
func (ts *Typescript) SerializeInOrder(sort func(nodes map[string]bindings.Node) []bindings.Node) (string, error) {
	...
	ts.sortImports()
	for _, imp := range ts.imports {
		obj, err := vm.ToTypescriptNode(imp)
		if err != nil {
			return "", fmt.Errorf("convert import %q: %w", imp.Module, err)
		}

		text, err := vm.SerializeToTypescript(obj)
		if err != nil {
			return "", fmt.Errorf("serialize import %q: %w", imp.Module, err)
		}
